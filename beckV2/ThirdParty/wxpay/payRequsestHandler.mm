...
//    NSString *order_price   = @"1";//1分钱测试
    ...
    [packageParams setObject: [Global sharedSingle].loginName        forKey:@"device_info"]; //支付设备号或门店号
    ...
    if ( prePayid != nil) {
        ...
        time_t now;
        time(&now);
        time_stamp  = [NSString stringWithFormat:@"%zd",(int) [[NSDate date] timeIntervalSince1970]/1000];
